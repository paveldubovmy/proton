...
def compute_all(lam, y_init=None):
    ...
    if not sol.success:
        print(f"  [WARN] λ={lam:.3f} — solve_bvp не сошёлся полностью")
    ...
    # Численная производная на логарифмической сетке: d/dr = (1/r) * d/d(ln r)
    ln_r   = np.log(r)
    r2pr   = r**2 * pr
    dr2pr  = np.gradient(r2pr, ln_r) / r   # = d(r^2 pr)/dr
    pt     = dr2pr / (2.0 * r)

    # Производные рассчитаны — теперь обрезаем сингулярность у r=0
    # Первые ~20 точек (r < 0.01 fm) артефактны из-за gradient — заменяем NaN
    ...
    # Лауэ (интегрируем только по надёжной зоне)
    mask_laue = r >= 0.01
...
# =========================================================
# БАЗОВОЕ РЕШЕНИЕ λ = 0.83 (калибровка по R_B = 0.8548 fm)
# =========================================================
print("Считаем базовое решение λ = 0.83 ...")
res = compute_all(0.83)
print(f"  R_B  = {res['RB']:.4f} fm   (цель: 0.8548 fm)")
print(f"  d1   = {res['d1']:.5f}")
print(f"  Лауэ = {res['laue']:.3e} fm^-1")
...
# МАСКИ ДЛЯ ГРАФИКОВ
# =========================================================
mask_plot  = (r >= 0.05) & (r <= 3.0)   # физический диапазон, без сингулярности
mask_laue  = (r >= 0.05) & (r <= 3.0)   # то же для Лауэ-интегранда
...
fig1.tight_layout()
fig1.savefig("fig1_profile_piso.png", dpi=300, bbox_inches='tight')
print("Сохранён fig1_profile_piso.png")
...
# Левая панель: компоненты давления
ax = axes[0]
...
ax.set_xlim(0, 3)
# Умный ylim: убираем хвостовые артефакты
...
fig2.tight_layout()
fig2.savefig("fig2_pressure_shear.png", dpi=300, bbox_inches='tight')
print("Сохранён fig2_pressure_shear.png")
...
for lam in lambda_values:
    ...
    print(f"  λ={lam:.2f}  d1={res_lam['d1']:9.5f}  "
          f"Лауэ={res_lam['laue']:10.3e}  R_B={res_lam['RB']:.4f} fm")

d1_values   = np.array(d1_values)
laue_values = np.array(laue_values)

# =========================================================
# РИС. 3: Лауэ-интегранд + d1(λ)
# =========================================================
fig3, axes = plt.subplots(1, 2, figsize=(13, 5))

# Левая панель: r^2 p_iso(r)
...
# Правая панель: d1(λ) с полосой JLab
ax = axes[1]
# Полоса JLab: d1 = -2.0 ± 0.4
ax.axhspan(-2.4, -1.6, alpha=0.15, color='green', label='JLab DVCS $(-2.0\\pm0.4)$')
...
# Отметить базовую точку λ=0.83
idx_base = np.argmin(abs(lambda_values - 0.83))
ax.plot(lambda_values[idx_base], d1_values[idx_base], '*',
        color='red', ms=14, zorder=5, label=r'$\lambda=0.83$ (калибровка)')
...
fig3.tight_layout()
fig3.savefig("fig3_laue_d1lambda.png", dpi=300, bbox_inches='tight')
print("Сохранён fig3_laue_d1lambda.png")

# =========================================================
# ИТОГОВАЯ ТАБЛИЦА
# =========================================================
print("\n" + "="*55)
print("  Таблица для статьи")
print("="*55)
print(f"  {'λ [fm]':>8}  {'d1':>10}  {'Лауэ [fm⁻¹]':>14}")
print("-"*55)
for lam, d1, laue in zip(lambda_values, d1_values, laue_values):
    marker = " ← калибровка" if abs(lam - 0.83) < 0.01 else ""
